...
def intro():
    ...
    print_sleep(f"Rumor has it that a {enemy} is somewhere around here, and has been terrifying the nearby village.")
    print_sleep("In front of you is a house.")
    print_sleep("To your right is a dark cave.")
    print_sleep(f"In your hand you hold your trusty (but not very effective) {weapon}.")
...
def combat(weapon: str) -> str:
    ...
    if weapon == "dagger":
        print_sleep(f"You feel a bit under-prepared for this, what with only having a tiny {weapon}.")
    ...
    if choice == '1':
        if weapon == "dagger":
            ...
            print_sleep(f"As the {enemy} moves to attack, you unsheath your new sword.")
            print_sleep(f"The Sword of Ogoroth shines brightly in your hand as you brace yourself for the attack.")
            print_sleep(f"But the {enemy} takes one look at your shiny new toy and runs away!")
            print_sleep(f"You have rid the town of the {enemy}. You are victorious!")
    elif choice == '2':
...
def cave():
    ...
    if cave_visited:
        ...
        print_sleep(f"You discard your silly old {weapon} and take the sword with you.")
        weapon = "sword"
    cave_visited = True
...
def house():
    ...
    print_sleep(f"You are about to knock when the door opens and out steps a {enemy}.")
